<template>
  <div class="app-container intake-cost">
    <!--筛选条件-->
      ...
      <el-table v-loading="listLoading" :key="tableKey" height="550" :data="list" border fit highlight-current-row @selection-change="selsChange" style="width: 100%;">
        ...
        <!-- <el-table-column label="取水记录ID" width="220" align="center">
                    <template slot-scope="scope">
                        <span class="link-type" @click="handleUpdate(scope.id)">{{ scope.row.id }}</span>
                    </template>
                </el-table-column> -->
        ...
        <el-table-column label="取水用途" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.use }}</span>
          </template>
        </el-table-column>
        <el-table-column label="月实际取水量" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.intoke }}</span>
          </template>
        </el-table-column>
        <el-table-column label="缴费频率" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.frequency }}</span>
          </template>
        </el-table-column>
        <el-table-column label="月缴费金额" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.pay }}</span>
          </template>
        </el-table-column>
        <el-table-column label="月计划取水量" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.plan }}</span>
          </template>
        </el-table-column>
        <el-table-column label="超计划取水比例" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.ratio }}</span>
          </template>
        </el-table-column>
        <el-table-column label="超累进收费比例" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.totalRatio }}</span>
          </template>
        </el-table-column>
        <el-table-column label="取水分析说明" align="center">
          <template slot-scope="scope">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="取水日期" prop="time">
                <el-date-picker v-model="temp.time" clearable type="month" placeholder="选择月" value-format="yyyy-MM-dd HH:mm:ss">
                </el-date-picker>
              </el-form-item>
              <el-form-item label="取水用途" prop="use">
                <el-input v-model="temp.use" clearable />
              </el-form-item>
              <el-form-item label="缴费频率" prop="frequency">
                <!-- <el-input v-model="temp.frequency" clearable /> -->
                <el-select v-model="temp.frequency" clearable value-key="department">
                  <el-option v-for="(val, key) in costFrequencyOptions" :key="key" :label="val" :value="key" />
                </el-select>
              </el-form-item>
              <el-form-item label="月计划取水量" prop="plan">
                <el-input v-model="temp.plan" clearable />
              </el-form-item>
              <el-form-item label="超累进收费比例" prop="totalRatio">
                <el-input v-model="temp.totalRatio" clearable />
              </el-form-item>
            ...
            <el-col :span="12">
              ...
              <el-form-item label="月实际取水量" prop="intoke">
                <el-input v-model="temp.intoke" clearable />
              </el-form-item>

              <el-form-item label="月缴费金额" prop="pay">
                <el-input v-model="temp.pay" clearable />
              </el-form-item>
              <el-form-item label="超计划取水比例" prop="ratio">
                <el-input v-model="temp.ratio" clearable />
              </el-form-item>
              <el-form-item label="取水分析说明" prop="analysis">
                <el-input v-model="temp.analysis" clearable />
              </el-form-item>
            </el-col>
          </el-row>
...
import {
  ...
  updateCost,
  getOrganStatusList,
  getCostFrequencyList
} from "@/commons/api/eneralize";

import { notify } from "@/commons/utils/notify";
import { intakeCostRules } from "@/commons/utils/validate";
import { checkNumber, parseTime } from "@/commons/utils";
...
export default {
  ...
  methods: {
    ...
    getCostFrequencyList() {
      getCostFrequencyList().then(response => {
        this.costFrequencyOptions = response.data;
      });
    },
    ...
    handleFilter() {
      ...
      if (this.listQuery.month) {
        ...
        this.temp.time = this.temp.time.split(" ")[0];
        this.temp.time =
          this.temp.time.split("-")[0] + "-" + this.temp.time.split("-")[1];
      } else {
        this.listQuery.time = "";
      }
      this.getList(); //刷新表格数据
    ...
    batchRemove: function() {
      ...
      this.$confirm("确认删除选中记录吗？", "提示", {
        ...
        .then(() => {
          var deleteCostInfo = { ids: ids };
          deleteCost(deleteCostInfo).then(response => {
    ...
    createData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          addCost(this.temp).then(response => {
            var res = notify(this, response);
            if (res) {
    ...
    //更新数据
    updateData() {
      if (checkNumber(this.temp.frequency)) {
      } else {
        this.temp.frequency = this.temp.frequencyId;
      }
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          updateCost(this.temp).then(response => {
            var res = notify(this, response);
            if (res) {
              this.dialogFormVisible = false;
              this.getList();
